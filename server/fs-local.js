...
let getMime = require('simple-mime')('application/octet-stream')
let vm = require('vm')
...
function canRead (owner, inGroup, mode) {
  return owner && (mode & 0o400) || // User is owner and owner can read.
    inGroup && (mode & 0o0040) || // User is in group and group can read.
    (mode & 0o0004) // Anyone can read.
}
function canWrite (owner, inGroup, mode) {
  return owner && (mode & 0o0200) || // User is owner and owner can write.
    inGroup && (mode & 0o0020) || // User is in group and group can write.
    (mode & 0o0002) // Anyone can write.
}
function canExec (owner, inGroup, mode) {
  return owner && (mode & 0o0100) || // User is owner and owner can exec.
    inGroup && (mode & 0o0010) || // User is in group and group can exec.
    (mode & 0o0001) // Anyone can write.
}
...
function checkType (vars, callback) {
  ...
  let errors = []
  for (let i = 0, l = vars.length; i < l; i += 3) {
    let name = vars[i]
    let value = vars[i + 1]
    let expectedType = vars[i + 2]
    let actualType = value === null ? 'null' : Array.isArray(value) ? 'array' : typeof value
...
module.exports = function setup (fsOptions) {
  ...
  let base = root.substr(0, root.length - 1)

  let umask = fsOptions.umask || 0o750
  let checkPermissions, fsUid, fsGid
    ...
    realpath(path, function (err, path) {
      ...
      fs.open(path, flags, mode, function (err, fd) {
        ...
        switch (flags) {
          ...
          case 'w+':
          case 'a+':
            requestedPermissions = 6
            break
          default:
            return callback(new Error('Invalid flag ' + flags))
        }
  ...
  function childrenOfPid (pid, callback) {
    exec('ps', {args: ['-A', '-oppid,pid']}, function (err, stdout, stderr) {
      ...
      stdout.split('\n').slice(1).forEach(function (line) {
        let col = line.trim().split(/\s+/g);
        (parents[col[0]] || (parents[col[0]] = [])).push(col[1])
      })

      function search (roots) {
        let res = roots.concat()
        for (let c, i = 0; i < roots.length; i++) {
  ...
  function connect (port, options, callback) {
    ...
    tryConnect()
    function tryConnect () {
      let socket = net.connect(port, function () {
  ...
  function readfile (path, options, callback) {
    ...
    open(path, 'r', umask & 0o666, function (err, path, fd, stat) {
      if (err) return callback(err)
      if (!stat.isFile()) {
        fs.close(fd)
        ...
        fs.readdir(path, function (err, files) {
          ...
          function getNext () {
            ...
            if (stat.access & 1/*EXEC/SEARCH*/) { // Can they enter the directory?
              ...
              let err = new Error('EACCESS: Permission Denied')
              err.code = 'EACCESS'
              onStatEntry({
                name: file,
                err: err
  ...
  function mkfile (path, options, callback) {
    ...
    realpath(dirname(path), function (err, dir) {
      ...
      let tmpPath = path + '.' + (Date.now() + Math.random() * 0x100000000).toString(36) + '.tmp'
      // node 0.8.x adds a "wx" shortcut, but since it's not in 0.6.x we use the
      // longhand here.
      let flags = constants.O_CREAT | constants.O_WRONLY | constants.O_EXCL
      fs.open(tmpPath, flags, umask & 0o666, function (err, fd) {
        if (err) return callback(err)
        options.fd = fd
        if (checkPermissions) {
        ...
        function onCreate () {
          let stream = new fs.WriteStream(path, options)
          let hadError
  ...
  function changedSince (paths, options, callback) {
    ...
    let since = (new Date(options.since)).getTime()

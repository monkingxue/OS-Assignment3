...
function newParser (req, str) {
  let boundary = new Buffer(str)
  return new Parser(req, boundary)
}

// Parser states
let DONE = 0x00
let BOUNDARY_START = 0x01
let BOUNDARY = 0x02
let BOUNDARY_END = 0x03
let HEADER_FIELD = 0x11
let HEADER_VALUE = 0x12
let HEADER_VALUE_END = 0x13
let HEADERS_ALMOST_DONE = 0x14
let PART_DATA = 0x21
let PART_END = 0x22
let END = 0x31
...
Parser.prototype.onData = function onData (chunk) {
  ...
  let start // offset within chunk of start of current piece
  let partStart // offset within chunk of data start
  let partEnd
  let leftover
  ...
  for (let i = 0; i < len; i++, this.offset++) {
    let c = chunk[i]
    ...
    switch (this.state) {
      ...
      case HEADER_FIELD:
        ...
        let field = chunk.toString('ascii', start, i)
      ...
      case HEADER_VALUE:
        ...
        let value = chunk.toString('ascii', start, i)
        if (leftover !== undefined) {
